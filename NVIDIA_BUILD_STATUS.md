...
- Local supported constructions remain fast and work without network access. The latest isolated local SVG-ready regression run measured p95 30.43 ms across 834 observations, **excluding speech recognition, browser commit/paint, and Android device scheduling**.
...
| Service suite | 14 passing | Mocked NVIDIA request, correction, invalid output, HTTP origin/health, key boundary, usage guards, provider throttling, and disconnect cancellation. |
