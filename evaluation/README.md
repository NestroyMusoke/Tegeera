...
(`call-return-flow`), 23 (`lifo-stack`), 24 and 28 (`condition-flow`), 25 (`indexed-row`), 26 (`progressive-narrowing`), 29 (`linked-chain`), 31 (`geometric-construction`), 32
...
batch is 3/24 strict, 24/24 automated-ready, and zero false-confident. All twenty-one implemented
...
3/24 strict, 24/24 automated-ready, and zero false-confident; all twenty-one implemented
...
`node scripts/visual-check.mjs` now generates `human-visual-review.html`, a twenty-one-scene,
...
`src/evaluation/constructionGeneralization.test.tsx` generates 110 accepted paraphrase
...
condition flows, and progressive narrowing. For every
...
Thirty paired near-misses cover missing semantic roles, wrong domains, negation,
uncertainty, duplicate routine stages, and incomplete sequences. They must all clarify.
The resulting 110/110 accepted and 30/30 safely clarified checkpoint is a broad development
regression test. It is not a hidden, independent, or real-teacher accuracy measurement and
does not replace the gold result of 3/24 strict, 24/24 automated-ready, and zero
false-confident. Human visual approval remains outstanding for all twenty-one drawings.
...
with the newer-family matrix, the checkpoint covers 246 supported variations and 49 safe
near-misses across all twenty specialist layouts.

## Local render-time evidence

The combined generated 246-input workload drives `runtimeEnvelope.test.tsx`, so performance
cannot be measured against easier phrases than semantic conformance. After warming all
paths, it records 738 samples and separates interpretation, validation, immutable scene
