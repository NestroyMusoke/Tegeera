...
function makePrompt(text, scene) {
  return `You are Tegeera's conservative semantic planner. Return JSON only, never markdown and never invent facts. Produce DoodleScript with schemaVersion "2.24.0", sceneId ${JSON.stringify(scene.sceneId)}, revision ${Number(scene.revision) + 1}, confidence 0..1, sourceText exactly ${JSON.stringify(text)}, and 1-30 commands. Entity kinds: ${kinds.join(", ")}. Directions: left, right, up, down. Relation kinds: ${relations.join(", ")}. Commands: create {entity:{id,kind,label?,x,y,scale,direction,highlighted}}, relate {relation:{id,kind,sourceIds,targetIds,objectIds?,predicate?,preposition?}}, move, update, remove, unrelate, clear. Coordinates x=6..94, y=12..88, scale=.5..2. Use unique IDs; reference only existing/new IDs. Use generic with a concise label for unknown concrete objects. Do not output unknown fields. Current scene: ${JSON.stringify(scene)}`;
}
