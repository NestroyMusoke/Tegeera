...
describe("versioned relation registry", () => {
  it("covers every schema relationship with a validated semantic contract", () => {
    expect(RELATION_REGISTRY_VERSION).toBe("2.16.0");
    expect(relationRegistry.map(({ kind }) => kind).sort()).toEqual([
      "accelerates", "accessedAt", "actsOn", "appliedTo", "away", "before", "calls", "carries", "causes", "contacts", "contains", "evaporatesTo", "fallsFrom", "fallsTo", "flowsFrom", "flowsInto", "flowsTo", "handover", "illuminates", "infiltrates", "measures", "opposes", "owns", "partOf", "pumpsTo", "pushesToward", "queuedFor", "reflectsFrom", "resultsIn", "returnsControlTo", "returnsTo", "risesTo", "routineBefore", "shares", "subtracts", "sumsTo", "toward", "transformsTo", "travelsTo", "trianglePartOf", "visualAction"
    ]);
