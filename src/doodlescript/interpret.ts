...
export function interpretTeacherText(input: string, scene: SceneState): Interpretation {
  ...
  try {
    ...
    for (let frameIndex = 0; frameIndex < semanticInput.frames.length; frameIndex += 1) {
      ...
      const processorMemory = frame.processorMemoryLinks[0];
      if (processorMemory?.construction === "processor-memory-link") {
        const mentionText = (mentionId: string) => [...frame.entities, ...frame.references].find((mention) => mention.mentionId === mentionId)?.text ?? "";
        const before = new Set(working.entities.map(({ id }) => id));
        const processorLabel = mentionText(processorMemory.processorMentionId);
        const memoryLabel = mentionText(processorMemory.memoryMentionId);
        const processorId = eventNode(processorLabel, "compute-unit", true);
        const memoryId = eventNode(memoryLabel, "memory-unit", true);
        for (const [id, label] of [[processorId, processorLabel], [memoryId, memoryLabel]] as const) {
          const created = commands.find((command) => command.action === "create" && command.entity.id === id);
          if (created?.action === "create") created.entity.label = label;
        }
        working = applyDoodleScript(scene, makeScript());
        if (processorId === memoryId) throw new Clarification("Processing and memory need distinct identities.", "conflicting-scene");
        const ids = { processorId, memoryId };
        const moves = planProcessorMemoryLink(working, ids, new Set(working.entities.filter(({ id }) => !before.has(id)).map(({ id }) => id)));
        if (!moves) throw new Clarification("That processing and memory link cannot fit readably.", "layout-limit");
        moves.forEach((move) => append({ action: "move", ...move }));
