...
describe("independent semantic-scene gold annotations", () => {
  ...
  it("validates a unique, cross-domain first annotation batch", () => {
    ...
    expect(parsed.cases.map(({ id }) => id)).toEqual([1, 2, 11, 12, 21, 22, 24, 28, 31, 32, 41, 42, 3, 4, 13, 23, 25, 29, 26, 27, 30, 33, 44, 51, 53, 56, 60]);
  ...
  it("reports every dimension honestly without requiring the current engine to pass", () => {
    const result = evaluateIndependentGold(teacherCases, gold, {
      1: observeCase(1), 2: observeCase(2), 3: observeCase(3), 4: observeCase(4), 11: observeCase(11), 12: observeCase(12), 13: observeCase(13), 21: observeCase(21), 22: observeCase(22), 23: observeCase(23), 24: observeCase(24), 25: observeCase(25), 26: observeCase(26), 27: observeCase(27), 28: observeCase(28), 29: observeCase(29), 30: observeCase(30), 31: observeCase(31), 32: observeCase(32), 33: observeCase(33), 41: observeCase(41), 42: observeCase(42), 44: observeCase(44), 51: observeCase(51)
    });
    ...
    expect(result.automatedReady).toBe(27);
  ...
  it.each([
    [4, "doubling-growth", ["single-origin", "two-offspring", "four-offspring", "eight-offspring", "doubling-branches", "exponential-counts"]],
    ...
    [27, "fifo-queue", ["fifo-box-lane", "rear-marker", "front-marker", "enqueue-arrow", "dequeue-arrow", "first-served-cue"]],
    [30, "processor-memory-link", ["processor-block", "memory-block", "bidirectional-data-bus", "speed-pulses", "nearby-spacing"]],
    ...
    [44, "convergent-plates", ["opposing-landmasses", "inward-force-arrows", "central-mountain-uplift"]],
    [51, "ordered-routine", ["wake-up-stage", "ready-stage", "school-stage", "ordered-arrows"]]
  ] as const)("makes case %i automated-ready through %s", (id, grammar, cues) => {
    const observation = observeCase(id);
    expect(new Set(observation.visualCueIds)).toEqual(new Set(cues));
    expect(observation.visualGrammarId).toBe(grammar);
    expect(evaluateIndependentGold(teacherCases, gold, { [id]: observation }).results.find((item) => item.id === id))
      .toMatchObject({ passed: false, falseConfident: false, automatedReady: true, failures: ["human visual review pending"] });
  });

  it("makes case 22 automated-ready while preserving human visual review", () => {
    const observation = observeCase(22);
    expect(new Set(observation.visualCueIds)).toEqual(new Set([
      "main-flow-line", "function-block", "call-arrow", "return-arrow", "same-return-point"
    ]));
  ...
  it("makes case 3 automated-ready while preserving human visual review", () => {
    const observation = observeCase(3);
    expect(new Set(observation.visualCueIds)).toEqual(new Set([
      "left-to-right-stages", "caterpillar-stage", "wrapped-cocoon", "emerging-butterfly"
    ]));
    expect(observation.visualGrammarId).toBe("lifecycle-sequence");
    expect(evaluateIndependentGold(teacherCases, gold, { 3: observation }).results.find(({ id }) => id === 3))
      .toMatchObject({ passed: false, falseConfident: false, automatedReady: true, failures: ["human visual review pending"] });
  });
});
