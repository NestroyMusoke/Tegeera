...
describe("generalized local render-time envelope", () => {
  it("keeps every accepted construction inside a coarse anti-regression budget", () => {
    const result = measureLocalRenderEnvelope(acceptedConstructionProbes, 3);
    console.info(
    ...
    expect(result.probeCount).toBe(66);
    expect(result.sampleCount).toBe(198);
  ...
  it("rejects an empty or invalid benchmark configuration", () => {
    ...
    expect(() => measureLocalRenderEnvelope(acceptedConstructionProbes, 0)).toThrow(/one repetition/);
  });
});
